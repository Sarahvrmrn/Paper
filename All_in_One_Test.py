...
def read_files(path: str, tag: str):
    ...
    threshold_percent = 1.5 # threshold in %
...
#     lda_loadings = lda.scalings_.flatten()

#     print(type(lda_loadings))
    
#     print(lda_loadings)
#     # Erstelle das Balkendiagramm
#     x_values = np.arange(len(lda_loadings))

#     # Erzeuge das Balkendiagramm
#     plt.bar(x_values, lda_loadings)

#     # Beschriftungen und Titel hinzufügen
#     plt.xlabel('Index')
#     plt.ylabel('Werte')
#     plt.title('Balkendiagramm der gegebenen Daten')
...
#     print(lda_loadings)
    ...
    # loadings_df = pd.DataFrame(data=lda_loadings, columns=[f'PC{i+1}' for i in range(len(lda_loadings))])
    ...
    #     fig.update_traces(marker=dict(size=4))
    #     # Save the plot as an HTML file
    #     fig.write_html(f"{column}_plot_Knolle_lda.html")
